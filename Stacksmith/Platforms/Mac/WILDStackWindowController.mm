...
-(void)	drawOneBoundingBox: (CPart*)currPart
{
    ...
    CVisiblePart* currVisiblePart = dynamic_cast<CVisiblePart*>(currPart);
    if( mStack->GetPeeking() || (currVisiblePart == nullptr || currVisiblePart->GetVisible()) )
    {
...
-(BOOL)	validateMenuItem: (NSMenuItem*)theItem
{
	...
	else if( theItem.action == @selector(projectMenuItemSelected:) )
	{
		...
		if( msg.length() > 0 && !currMenuItem->HasOrInheritsMessageHandler( msg.c_str(), nullptr, nullptr) )
			return NO;
		else if( msg.length() == 0 && !currMenuItem->HasOrInheritsMessageHandler( "doMenu", nullptr, nullptr ) )
			return NO;
