		...
		else if( theDestination )
		{
			...
			if( !destinationObject )
			{
				...
				[inContext,overPartObject,userData]( CDocument * inNewDocument )
				{
					...
					LEOContextRelease(inContext);
				}, userData->GetVisualEffectType(), userData->GetVisualEffectSpeed(), inContext->group, EOpenVisibly );
...
struct THostCommandEntry	gStacksmithHostCommands[] =
{
	...
	},
	{
		...
		{
			{ EHostParamExpressionOrConstant, EToolIdentifier, EHostParameterRequired, INVALID_INSTR2, 0, 0, '\0', '\0' },
