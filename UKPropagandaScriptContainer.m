...
@implementation UKPropagandaSymbol
...
-(id)	initWithLine: (NSInteger)lineIdx symbolName: (NSString*)inName
			symbolType: (UKPropagandaSymbolType)inType
{
	if(( self = [super init] ))
	{
		lineIndex = lineIdx;
...
NSString*	UKPropagandaFormatScript( NSString* scriptString, NSArray* *outSymbols )
{
	...
	while( YES )
	{
		...
		if( [scanny scanString: @"on" intoString: nil] )
		{
			...
			{
				UKPropagandaSymbol*	sym = [[UKPropagandaSymbol alloc] initWithLine: currentLine
												symbolName: theName
												symbolType: UKPropagandaSymbolTypeHandler];
		...
		else if( [scanny scanString: @"function" intoString: nil] )
		{
			...
			if( [scanny scanCharactersFromSet: idCS intoString: &theName] )
			{
				UKPropagandaSymbol*	sym = [[UKPropagandaSymbol alloc] initWithLine: currentLine
												symbolName: theName
												symbolType: UKPropagandaSymbolTypeFunction];
				[symbols addObject: sym];
				[sym release];
				[openBlockNames addObject: theName];
				addToIndentationAfterThisLine++;
